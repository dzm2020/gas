...
type Component struct {
	...
	node   iface.INode
	// 存储启动的 actor 进程
	processes map[string]iface.IProcess
	mu        sync.RWMutex
}
...
func New(name string, config *Config, node iface.INode) *Component {
	...
	return &Component{
		...
		config:    config,
		node:      node,
		processes: make(map[string]iface.IProcess),
	}
}
...
func (c *Component) Start(ctx context.Context) error {
	...
	for _, actorConfig := range c.config.Actors {
		...
		c.processes[actorConfig.Name] = process
		glog.Infof("app: actor '%s' started with pid=%d", actorConfig.Name, pid.GetServiceId())
	}

	glog.Infof("app: started %d actors", len(c.processes))
	return nil
}
...
func (c *Component) Stop(ctx context.Context) error {
	...
	for name, process := range c.processes {
		if err := process.Exit(); err != nil {
			glog.Errorf("app: exit actor '%s' failed: %v", name, err)
			lastErr = err
		} else {
			glog.Infof("app: actor '%s' stopped", name)
		}
	}

	c.processes = make(map[string]iface.IProcess)
	return lastErr
}
