...
func TestGate(t *testing.T) {
	t.Run("OnConnect_MaxConn", func(t *testing.T) {
		...
		if err == nil { t.Fatal("want error when count > MaxConn") }
		if g.count.Load() != 2 { t.Errorf("count=%d", g.count.Load()) }
	})
	t.Run("OnConnect_BindsSession", func(t *testing.T) {
		...
		g.Factory = func() IAgent { return &Agent{} }
		conn := &mockConn{id: 99}
		if err := g.OnConnect(conn); err != nil { t.Fatal(err) }
		s, ok := conn.Context().(*session.Session)
		if !ok || s == nil || s.GetEntityId() != 99 || s.GetAgent() == nil { t.Fatalf("session not bound: %T %v", conn.Context(), conn.Context()) }
	})
	t.Run("OnMessage", func(t *testing.T) {
		g := &Gate{system: &mockSystem{}}
		conn := &mockConn{id: 1}
		n, err := g.OnMessage(conn, []byte{0, 0, 0})
		if err != nil || n != 0 { t.Fatalf("short: n=%d err=%v", n, err) }
		enc, _ := codec.Encode(protocol.New(1, 2, []byte("x")))
		n, err = g.OnMessage(conn, enc)
		if err != nil || n != len(enc) { t.Fatalf("no session: n=%d err=%v", n, err) }
		s := session.New(1, nil)
		conn.context = s
		n, err = g.OnMessage(conn, enc)
		if err != nil || n != len(enc) { t.Fatalf("session no agent: n=%d err=%v", n, err) }
	})
	t.Run("OnClose", func(t *testing.T) {
		...
		if g.count.Load() != 0 || sys.shutdownProcess != nil { t.Fatalf("no session: count=%d shutdown=%v", g.count.Load(), sys.shutdownProcess) }
		pid := &iface.Pid{ServiceId: 42}
		g.count.Store(1)
		g.OnClose(&mockConn{id: 1, context: session.New(1, pid)}, nil)
		if g.count.Load() != 0 || sys.shutdownProcess != pid { t.Fatalf("with session: shutdown=%v", sys.shutdownProcess) }
	})
	t.Run("Stop", func(t *testing.T) {
		if err := (&Gate{}).Stop(context.Background()); err != nil { t.Fatal(err) }
	})
}
...
func TestAgent(t *testing.T) {
	t.Run("Push", func(t *testing.T) {
		...
		if err := (&Agent{}).Push(testContext{}, s, []byte("hello")); err != nil { t.Fatal(err) }
		if len(conn.sent) != 1 { t.Fatalf("Send %d times", len(conn.sent)) }
		exp := protocol.New(1, 2, []byte("hello"))
		exp.Index = 3
		enc, _ := codec.Encode(exp)
		if !bytes.Equal(conn.sent[0], enc) { t.Errorf("sent mismatch") }
	})
	t.Run("Push_NoConn", func(t *testing.T) {
		network.ClearConnections()
		defer network.ClearConnections()
		err := (&Agent{}).Push(testContext{}, session.New(999, &iface.Pid{}), []byte("x"))
		if err == nil || !errors.Is(err, ErrNotFoundEntity) { t.Fatalf("err=%v", err) }
	})
	t.Run("Push_Middleware", func(t *testing.T) {
		...
		if err := agent.Push(testContext{}, session.New(1, &iface.Pid{}), []byte("body")); err != nil { t.Fatal(err) }
		msg, _, _ := codec.Decode(conn.sent[0])
		if string(msg.Data) != "body-ok" { t.Errorf("data=%q", msg.Data) }
	})
	t.Run("Shutdown", func(t *testing.T) {
		if err := (&Agent{}).Shutdown(testContext{}, nil); err != nil { t.Fatal(err) }
		network.ClearConnections()
		defer network.ClearConnections()
